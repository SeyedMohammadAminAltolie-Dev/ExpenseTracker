...
final class TransactionListViewModel: ObservableObject {
    ...
    func accumulateTransactions() -> TransactionPrefixSum {
        ...
        let dateIntervarl = Calendar.current.dateInterval(of: .month, for: today)!
        print("dateInterval", dateIntervarl)
        
        var sum: Double = .zero
        var cumulativeSum = TransactionPrefixSum()
        
        for date in stride(from: dateIntervarl.start, to: today, by: 60 * 60 * 24) {
